...
def parse_args():
    ...
    parser.add_argument(
        "--raw_path",
        type=str,
        default=str(RAW_STOCK_PANEL_PATH),
        help="Path to stock_panel_raw.csv"
    )
    parser.add_argument(
        "--out_dir",
        type=str,
        default=str(CLEANED_DATA_DIR),
...
def load_raw_dataframe(path: Path) -> pd.DataFrame:
    if not path.exists():
        raise FileNotFoundError(
            ...
            "请先运行 data_engineering_agent_revised.py 生成 stock_panel_raw.csv"
        )
...
def main():
    ...
    save_dataset(clean_base, out_dir / CLEAN_BASE_DATA.name)
    save_dataset(main_h1, out_dir / MAIN_H1_DATA.name)
    save_dataset(main_h5, out_dir / MAIN_H5_DATA.name)
    save_dataset(ext_h1, out_dir / EXT_H1_DATA.name)
    save_dataset(ext_h5, out_dir / EXT_H5_DATA.name)
    ...
    with open(out_dir / FEATURE_CONFIG_PATH.name, "w", encoding="utf-8") as f:
    ...
    with open(out_dir / DATASET_SUMMARY_PATH.name, "w", encoding="utf-8") as f:
    ...
    print("\nSaved files:")
    print(out_dir / CLEAN_BASE_DATA.name)
    print(out_dir / MAIN_H1_DATA.name)
    print(out_dir / MAIN_H5_DATA.name)
    print(out_dir / EXT_H1_DATA.name)
    print(out_dir / EXT_H5_DATA.name)
    print(out_dir / FEATURE_CONFIG_PATH.name)
    print(out_dir / DATASET_SUMMARY_PATH.name)
    print("\n🎉 数据集准备完成。下一步进入 scripts/training/train_transformer_main.py")
