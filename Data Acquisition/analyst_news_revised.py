...
class NewsAnalyst:
    """
    Current dissertation-friendly proxy version.
    It does not use NLP headlines yet; instead it builds macro/news-pressure proxies
    from liquid market instruments that often react immediately to macro news.

    If later you add a real news API, this file is the right place to replace these proxies.
    """
    ...
    def run(self, start: str, end: str) -> pd.DataFrame:
        ...
        rate_z = (df["rate_change_5d"] - df["rate_change_5d"].rolling(60, min_periods=20).mean()) / (
            df["rate_change_5d"].rolling(60, min_periods=20).std() + 1e-6
        )
        stress_z = (df["credit_stress"] - df["credit_stress"].rolling(60, min_periods=20).mean()) / (
            df["credit_stress"].rolling(60, min_periods=20).std() + 1e-6
        )
