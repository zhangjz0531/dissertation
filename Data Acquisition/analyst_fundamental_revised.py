...
class FundamentalAnalyst:
    """
    Download daily OHLCV and align quarterly fundamentals to daily rows.
    Notes:
    1) We use an effective date with a filing lag to reduce look-ahead bias.
    2) We avoid filling missing fundamentals with arbitrary constants like 0.10.
    3) Features are intentionally simple and robust for dissertation experiments.
    """
    ...
    def _load_quarterly_fundamentals(self, ticker: str) -> pd.DataFrame:
        ...
        net_income = self._pick_first_existing(
            fund,
            [
                "Net Income",
        ...
        total_debt = self._pick_first_existing(
            fund,
            [
                "Total Debt",
                "Long Term Debt",
        ...
        total_equity = self._pick_first_existing(
            fund,
            [
                "Stockholders Equity",
    ...
    def _merge_price_and_fundamental(self, price_df: pd.DataFrame, fund_df: pd.DataFrame) -> pd.DataFrame:
        ...
        merged = pd.merge_asof(
            price_df,
            fund_df.sort_values("effective_date"),
            left_on="date",
    ...
    def run(self, start: str, end: str, stocks: List[str], macro: str) -> pd.DataFrame:
        ...
        for ticker in stocks:
            try:
                ...
                merged = self._merge_price_and_fundamental(price_df, fund_df)
