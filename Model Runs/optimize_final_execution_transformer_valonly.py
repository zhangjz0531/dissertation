...
# Important fix
# -------------
# This version removes ALL test metrics from the parameter search score.
# Parameters are selected using validation only, then evaluated once on test.
#
# Goal
# ----
# Improve final-system behavior without retraining:
                ...
                for switch_buffer in switch_buffer_grid:
                    for min_hold in min_hold_grid:
                        for max_new in max_new_names_grid:
                            ...
                            # VALIDATION-ONLY score: no test data in selection
                            score = (
...
def main():
    parser = argparse.ArgumentParser(description="Optimize final deployable execution on fixed Transformer predictions (validation-only).")
    ...
    out_root = Path(args.out_dir) / "final_execution_optimization_valonly"
