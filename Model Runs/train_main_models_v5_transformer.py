...
def build_sequences(df: pd.DataFrame, feats: List[str], target_col: str, return_col: str, seq_len: int, split_col: str = "split") -> Tuple[np.ndarray, np.ndarray, pd.DataFrame]:
    X_list, y_list, meta = [], [], []
...
def run_strategy_backtest(pred_df: pd.DataFrame, horizon: int, mode: str, top_k: int, min_prob: float, threshold: float, transaction_cost_bps: float) -> Dict[str, float]:
    if pred_df.empty:
        return {"periods": 0, "cumulative_return": 0.0, "annualized_return": 0.0, "sharpe": 0.0, "max_drawdown": 0.0, "win_rate": 0.0, "avg_turnover": 0.0, "avg_holdings": 0.0}
    df = pred_df.copy()
...
def choose_strategy_on_validation(val_pred_df: pd.DataFrame, horizon: int, mode_grid: List[str], topk_grid: List[int], min_prob_grid: List[float], threshold_grid: List[float], transaction_cost_bps: float, max_val_drawdown: float, score_weights: Dict[str, float], auc_for_epoch: float) -> Tuple[Dict, Dict, pd.DataFrame]:
    rows = []
...
def train_transformer_main(data_path: str, out_dir: str, args):
    ...
    print(f"using fixed pos_weight for BCE: {args.pos_weight:.4f}")
    ...
    for epoch in range(1, args.max_epochs + 1):
        ...
        print(
            f"[transformer_v5] epoch {epoch:02d} | lr={lr_now:.6f} | "
        ...
        if epoch >= args.min_epochs and wait >= args.patience:
            print(
                f"[transformer_v5] early stopping triggered at epoch {epoch}. "
                f"Best strategy epoch={best_strategy_epoch}, best_strategy_score={best_strategy_score:.6f}, "
                f"best_val_auc_epoch={best_auc_epoch}, best_val_auc={best_auc:.6f}"
            )
            break

    model_run_name = f"{Path(data_path).stem}_v5_transformer_seq{args.seq_len}_lr{args.lr}_wd{args.weight_decay}_do{args.dropout}"
    save_dir = Path(out_dir) / model_run_name
    ...
    strat_train_prob = predict_probs(model, DataLoader(train_ds, batch_size=args.batch_size, shuffle=False), device)
...
def main():
    parser = argparse.ArgumentParser(description="Optimize Transformer main system for the dissertation.")
