...
# =========================================================
# Final dissertation dashboard for the OPTIMIZED system
#
# Final system:
#   H5 Transformer v5 fixed predictions
#   + validation-only optimized sticky execution
#
# This dashboard reads the already-frozen final optimized system:
#   final_transformer_optimized_system/
...
DEFAULT_OUT_HTML = r"D:\python\dissertation\Model Runs\final_run_20260413\dissertation_trading_dashboard_transformer_optimized_final.html"
...
def load_inputs(
    ...
    snapshot_latest = safe_read_csv(str(final_dir / "final_snapshot_latest.csv"))
    ...
    if "date" in snapshot_latest.columns:
        snapshot_latest["date"] = pd.to_datetime(snapshot_latest["date"])
    ...
    return manifest, metrics, val_actions, test_actions, snapshot_latest, pred
...
def render_dashboard(
    ...
    latest_top = snapshot_latest.sort_values("pred_prob", ascending=False).copy()
    cols_top = [c for c in ["stock", "pred_prob", "selected_in_final", "final_weight"] if c in latest_top.columns]
    latest_top = latest_top[cols_top].head(8).copy()
    if "pred_prob" in latest_top.columns:
        latest_top["pred_prob"] = latest_top["pred_prob"].map(lambda x: f"{float(x):.4f}")
    if "final_weight" in latest_top.columns:
        latest_top["final_weight"] = latest_top["final_weight"].map(lambda x: f"{float(x):.4f}")

    market_cols = [c for c in ["stock", "dc_trend", "mkt_dc_trend", "vix_z_60", "credit_stress"] if c in snapshot_latest.columns]
    market_snapshot = snapshot_latest[market_cols].head(1).copy() if market_cols else pd.DataFrame()

    recent_actions = test_actions[[
        c for c in ["date", "selected_stocks", "n_holdings", "net_return", "turnover", "equity", "drawdown", "exposure", "newly_added", "kept_existing"]
        if c in test_actions.columns
    ]].tail(12).sort_values("date", ascending=False).copy()
...
<div class="section">
  <h2>Latest Ranked Stocks and Final Weights</h2>
...
def main():
    parser = argparse.ArgumentParser(description="Generate final dashboard for optimized Transformer system.")
    ...
    manifest, metrics, val_actions, test_actions, snapshot_latest, pred = load_inputs(
