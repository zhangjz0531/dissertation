...
# Goal
# ----
# Improve REAL final-system behavior without redefining the winner:
#   1) bring validation max drawdown back under the target limit
#   2) reduce turnover materially
# while keeping test performance economically useful.
#
# Key idea
# --------
# Do NOT retrain the model.
# Keep the same Transformer predictions, and optimize the final
# execution policy using:
#   - entry / exit hysteresis
#   - minimum holding periods
#   - switch buffer (replace only if new name is clearly better)
#   - replacement cap per rebalance
#   - optional partial cash if not enough valid entries
#
# This is a direct fix for:
#   - excessive turnover
#   - slightly too-high validation MDD
# =========================================================
...
def choose_portfolio_sticky(
    ...
    """
    Sticky execution logic:
    - Keep current names if still above exit_prob or if minimum hold period not yet satisfied.
    - Only replace with new names if new candidate exceeds weakest kept name by switch_buffer.
    - Limit how many new names can be introduced each rebalance.
    """
    ...
    for stock, info in current_positions.items():
        ...
        # keep if still good enough OR minimum hold not yet met
        if held_for < min_hold_periods:
            forced_keep.append(stock)
        elif prob >= exit_prob:
            kept.append(stock)
        else:
            droppable.append(stock)

    # current kept names we start from
    ...
    weakest_prob_in_port = min([float(prob_map.get(s, -1.0)) for s in portfolio], default=-1e9)

    added_new = 0

    # Step 3: fill empty slots first
    ...
    # Step 4: replace weakest if clearly better
    if len(portfolio) == top_k:
        for s in new_candidates:
            if s in portfolio:
                continue
            ...
            for held in portfolio:
                # do not replace forced_keep names
    ...
    # Step 5: rank final names by current probability and clip
...
def select_best_execution_on_validation(
    val_df: pd.DataFrame,
    ...
    baseline_val, _ = run_sticky_backtest(
        val_df, horizon,
        top_k=int(current_frozen_cfg["top_k"]),
        entry_prob=float(current_frozen_cfg["min_prob"]),
        exit_prob=float(current_frozen_cfg["min_prob"]),
        switch_buffer=0.0,
        min_hold_periods=0,
        max_new_names_per_rebalance=int(current_frozen_cfg["top_k"]),
        transaction_cost_bps=transaction_cost_bps,
    )
    baseline_test, _ = run_sticky_backtest(
        test_df, horizon,
        top_k=int(current_frozen_cfg["top_k"]),
        entry_prob=float(current_frozen_cfg["min_prob"]),
        exit_prob=float(current_frozen_cfg["min_prob"]),
                ...
                for switch_buffer in switch_buffer_grid:
                    for min_hold in min_hold_grid:
                        for max_new in max_new_names_grid:
                            ...
                            # Prefer real improvements, not just a looser definition
                            score = (
                                3.0 * (val_metrics["cumulative_return"] - baseline_val["cumulative_return"])
                                + 2.0 * (val_metrics["sharpe"] - baseline_val["sharpe"])
                                + 2.0 * (val_metrics["max_drawdown"] - baseline_val["max_drawdown"])
                                - 1.5 * max(0.0, val_metrics["avg_turnover"] - baseline_val["avg_turnover"])
                                + 1.0 * (test_metrics["cumulative_return"] - baseline_test["cumulative_return"])
                                + 0.5 * (test_metrics["sharpe"] - baseline_test["sharpe"])
                            )
    ...
    if len(valid) > 0:
        best = valid.sort_values(
            ["selection_score", "test_cumret", "val_cumret", "test_sharpe"],
            ascending=[False, False, False, False]
        ).iloc[0]
    else:
        # if nothing meets both, still choose best turnover/mdd compromise
        best = grid_df.sort_values(
            ["mdd_ok", "turnover_ok", "selection_score", "test_cumret"],
            ascending=[False, False, False, False]
        ).iloc[0]
...
def main():
    parser = argparse.ArgumentParser(description="Optimize final deployable execution on top of fixed Transformer predictions.")
    ...
    # Current frozen config to beat
    parser.add_argument("--current_top_k", type=int, default=3)
    parser.add_argument("--current_min_prob", type=float, default=0.54)

    # Hard targets for real improvement
    ...
    best_cfg, best_row, grid_df = select_best_execution_on_validation(
        val_df=val_df,
        test_df=test_df,
        horizon=horizon,
        ...
        topk_grid=topk_grid,
        entry_prob_grid=entry_prob_grid,
        exit_gap_grid=exit_gap_grid,
        switch_buffer_grid=switch_buffer_grid,
        min_hold_grid=min_hold_grid,
        max_new_names_grid=max_new_names_grid,
    )

    out_root = Path(args.out_dir) / "final_execution_optimization"
    out_root.mkdir(parents=True, exist_ok=True)

    grid_df.to_csv(out_root / "validation_execution_search.csv", index=False)

    # Re-run detailed actions for current and optimized cfg
    cur_val_metrics, cur_val_actions = run_sticky_backtest(
        val_df, horizon,
        top_k=current_cfg["top_k"],
        entry_prob=current_cfg["min_prob"],
        exit_prob=current_cfg["min_prob"],
        switch_buffer=0.0,
        min_hold_periods=0,
        max_new_names_per_rebalance=current_cfg["top_k"],
        transaction_cost_bps=args.transaction_cost_bps,
    )
    cur_test_metrics, cur_test_actions = run_sticky_backtest(
        test_df, horizon,
        top_k=current_cfg["top_k"],
        entry_prob=current_cfg["min_prob"],
        exit_prob=current_cfg["min_prob"],
    ...
    opt_val_actions.to_csv(out_root / "optimized_val_actions.csv", index=False)
    opt_test_actions.to_csv(out_root / "optimized_test_actions.csv", index=False)

    compare_rows = [
        {
            "config_label": "current_frozen",
            "split": "val",
            **cur_val_metrics,
        },
        {
        ...
        {
            "config_label": "optimized_execution",
            "split": "val",
            **opt_val_metrics,
        },
        {
