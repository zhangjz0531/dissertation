...
# =========================================================
# Final Transformer v5 full-sample walk-forward evaluator
#
# Why this script exists
# ----------------------
# It addresses the 4 most important concerns around the final
# Transformer v5 main system:
#
# 1) Coverage incomplete:
#    default folds now cover the sample as far forward as possible,
#    instead of stopping at 2021.
#
# 2) Dispersion / regime sensitivity:
#    outputs fold-level, year-level, and regime-bucket summaries.
#
# 3) Parameter variation:
#    reports how often each selected (top_k, min_prob) pair is chosen,
#    and compares the adaptive selected strategy against a locked
#    "final practical config".
#
# 4) Turnover / transaction-cost risk:
#    reports 5/10/20 bps cost sensitivity for both the adaptive
#    selected strategy and the locked final config.
#
# Suggested first run
# -------------------
# python train_transformer_v5_walkforward_fullsample.py ^
...
# test_days  = 126   (~6m)
# step_days  = 126   (~6m)
# max_folds  = 0     (0 means: use all possible folds)
#
# This is chosen specifically to push coverage into the late sample
# (2024-2026) while keeping chronological walk-forward discipline.
# =========================================================
...
def make_walkforward_folds(
    ...
    """
    max_folds=0 means use all possible folds.
    """
    n = len(unique_dates)
    folds = []
    fold_id = 1
...
def collect_cost_metrics(
    ...
    for cost in cost_grid_bps:
        ...
        tag = str(int(cost)) if float(cost).is_integer() else str(cost).replace(".", "_")
        out[f"{prefix}_cost_{tag}_cumret"] = float(stats["cumulative_return"])
        out[f"{prefix}_cost_{tag}_sharpe"] = float(stats["sharpe"])
        out[f"{prefix}_cost_{tag}_mdd"] = float(stats["max_drawdown"])
        out[f"{prefix}_cost_{tag}_turnover"] = float(stats["avg_turnover"])
    return out
...
def run_one_fold(
    ...
    # Evaluate best strategy checkpoint
    ...
    # Locked final practical config benchmark on same predictions
    fixed_val_stats = run_strategy_backtest(
        pred_df=strat_val_pred_df,
        horizon=horizon,
        mode=args.fixed_mode,
        top_k=args.fixed_top_k,
        min_prob=args.fixed_min_prob,
        threshold=args.fixed_threshold,
        transaction_cost_bps=args.transaction_cost_bps,
    )
    fixed_test_stats = run_strategy_backtest(
        pred_df=strat_test_pred_df,
        horizon=horizon,
        mode=args.fixed_mode,
        top_k=args.fixed_top_k,
    ...
    cost_grid = args.cost_grid_list
    selected_cost_metrics = collect_cost_metrics(
        pred_df=strat_test_pred_df,
        horizon=horizon,
        mode=best_strategy_cfg["mode"],
        top_k=best_strategy_cfg["top_k"],
        ...
        prefix="selected_test",
    )
    fixed_cost_metrics = collect_cost_metrics(
        pred_df=strat_test_pred_df,
        horizon=horizon,
        mode=args.fixed_mode,
        top_k=args.fixed_top_k,
    ...
    summary = {
        ...
        "fixed_min_prob": args.fixed_min_prob,
        "fixed_threshold": args.fixed_threshold,
        "selected_matches_fixed": bool(
            best_strategy_cfg["mode"] == args.fixed_mode and
            best_strategy_cfg["top_k"] == args.fixed_top_k and
            abs(best_strategy_cfg["min_prob"] - args.fixed_min_prob) < 1e-12 and
            abs(best_strategy_cfg["threshold"] - args.fixed_threshold) < 1e-12
        ),
        ...
        "fixed_val_cumulative_return": float(fixed_val_stats["cumulative_return"]),
        "fixed_val_sharpe": float(fixed_val_stats["sharpe"]),
        "fixed_val_max_drawdown": float(fixed_val_stats["max_drawdown"]),
        "fixed_val_avg_turnover": float(fixed_val_stats["avg_turnover"]),

        "fixed_test_cumulative_return": float(fixed_test_stats["cumulative_return"]),
        "fixed_test_sharpe": float(fixed_test_stats["sharpe"]),
        "fixed_test_max_drawdown": float(fixed_test_stats["max_drawdown"]),
        "fixed_test_avg_turnover": float(fixed_test_stats["avg_turnover"]),
    }
    summary.update(selected_cost_metrics)
    summary.update(fixed_cost_metrics)
...
def summarize_by_group(fold_df: pd.DataFrame, group_col: str) -> pd.DataFrame:
    ...
    for group_value, g in fold_df.groupby(group_col):
        rows.append({
            ...
            "fixed_test_cumret_mean": float(g["fixed_test_cumulative_return"].mean()),
            "fixed_test_sharpe_mean": float(g["fixed_test_sharpe"].mean()),
            "fixed_test_mdd_mean": float(g["fixed_test_max_drawdown"].mean()),
            "selected_matches_fixed_rate": float(g["selected_matches_fixed"].mean()),
        })
    return pd.DataFrame(rows)


def build_cost_sensitivity_summary(fold_df: pd.DataFrame, cost_grid: List[float]) -> pd.DataFrame:
    rows = []
    for label_prefix in ["selected_test", "fixed_test"]:
        for cost in cost_grid:
            tag = str(int(cost)) if float(cost).is_integer() else str(cost).replace(".", "_")
            rows.append({
                "strategy_type": label_prefix,
                "cost_bps": cost,
                "cumret_mean": float(fold_df[f"{label_prefix}_cost_{tag}_cumret"].mean()),
                "cumret_std": float(fold_df[f"{label_prefix}_cost_{tag}_cumret"].std(ddof=1)) if len(fold_df) > 1 else 0.0,
                "sharpe_mean": float(fold_df[f"{label_prefix}_cost_{tag}_sharpe"].mean()),
                "sharpe_std": float(fold_df[f"{label_prefix}_cost_{tag}_sharpe"].std(ddof=1)) if len(fold_df) > 1 else 0.0,
                "mdd_mean": float(fold_df[f"{label_prefix}_cost_{tag}_mdd"].mean()),
                "turnover_mean": float(fold_df[f"{label_prefix}_cost_{tag}_turnover"].mean()),
            })
    return pd.DataFrame(rows)
...
# -----------------------------
def main():
    parser = argparse.ArgumentParser(description="Full-sample walk-forward evaluation for the final Transformer v5 main system.")
    parser.add_argument("--data_path", type=str, required=True)
    parser.add_argument("--out_dir", type=str, required=True)

    # Fold plan: defaults chosen to push coverage into 2026
    ...
    # Strategy selection grid (adaptive strategy)
    ...
    if len(folds) == 0:
        raise ValueError("No valid folds were created. Try smaller train/val/test windows.")

    run_name = (
        f"{Path(args.data_path).stem}_transformer_v5_fullsample_walkforward_"
    ...
    overall = {
        ...
        "positive_fixed_test_cumret_rate": float((fold_df["fixed_test_cumulative_return"] > 0).mean()),
        ...
        "fixed_test_cumret_mean": float(fold_df["fixed_test_cumulative_return"].mean()),
        "fixed_test_sharpe_mean": float(fold_df["fixed_test_sharpe"].mean()),
        "fixed_test_mdd_mean": float(fold_df["fixed_test_max_drawdown"].mean()),
        "selected_matches_fixed_rate": float(fold_df["selected_matches_fixed"].mean()),
    }
    ...
    metric_cols = [
        ...
        "fixed_val_cumulative_return",
        "fixed_val_sharpe",
        "fixed_val_max_drawdown",
        "fixed_val_avg_turnover",
        "fixed_test_cumulative_return",
        "fixed_test_sharpe",
        "fixed_test_max_drawdown",
        "fixed_test_avg_turnover",
    ]
    ...
    # Cost sensitivity
    cost_summary_df = build_cost_sensitivity_summary(fold_df, args.cost_grid_list)
    cost_summary_df.to_csv(out_root / "cost_sensitivity_summary.csv", index=False)

    print("\nFinished full-sample walk-forward evaluation.")
