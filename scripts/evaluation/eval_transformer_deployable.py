...
from project_paths import MAIN_H5_DATA, EVALUATION_DIR, ensure_all_core_dirs, ensure_dir, timestamp_tag


# =========================================================
# Final Transformer deployable diagnostics
# =========================================================
...
def run_one_fold(
    ...
    if best_strategy_state is None or best_strategy_cfg is None:
        raise RuntimeError(f"Fold {fold['fold_id']} failed to produce a valid strategy checkpoint.")
    ...
    strat_val_out = strat_val_pred_df.copy()
    strat_val_out["date"] = pd.to_datetime(strat_val_out["date"]).dt.strftime("%Y-%m-%d")
    strat_test_out = strat_test_pred_df.copy()
    strat_test_out["date"] = pd.to_datetime(strat_test_out["date"]).dt.strftime("%Y-%m-%d")
    strat_val_out.to_csv(fold_dir / "val_predictions.csv", index=False)
    strat_test_out.to_csv(fold_dir / "test_predictions.csv", index=False)
...
def aggregate_candidate_grid(all_candidate_df: pd.DataFrame, max_val_drawdown: float) -> pd.DataFrame:
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Final Transformer deployable diagnostics.")
    parser.add_argument("--data_path", type=str, default=str(MAIN_H5_DATA))
    parser.add_argument("--out_dir", type=str, default=str(EVALUATION_DIR / "transformer_deployable"))
    ...
    parser.add_argument("--walk_mode", type=str, default="expanding")
    ...
    parser.add_argument("--cost_grid_bps", type=str, default="5,10,20")
    ...
    args.cost_grid_list = [float(x.strip()) for x in args.cost_grid_bps.split(",") if x.strip()]
    ...
    unique_dates = sorted(raw_df["date"].drop_duplicates().tolist())
    ...
    if len(folds) == 0:
        raise RuntimeError("No walk-forward folds were generated. Please reduce fold sizes.")
    ...
    for fold in folds:
        fold_dir = save_dir / f"fold_{int(fold['fold_id']):02d}"
        ...
        summary, candidate_df, cost_df = run_one_fold(
            raw_df=raw_df,
            feats=feats,
        ...
        all_cost_frames.append(cost_df)

    fold_df = pd.DataFrame(fold_summaries)
    fold_df.to_csv(save_dir / "fold_summaries.csv", index=False)

    all_candidate_df = pd.concat(all_candidate_frames, ignore_index=True) if all_candidate_frames else pd.DataFrame()
    all_cost_df = pd.concat(all_cost_frames, ignore_index=True) if all_cost_frames else pd.DataFrame()

    if len(all_candidate_df) > 0:
        all_candidate_df.to_csv(save_dir / "all_candidate_grid_metrics.csv", index=False)
    if len(all_cost_df) > 0:
        all_cost_df.to_csv(save_dir / "all_cost_sensitivity.csv", index=False)
    ...
    overall_metric_summary = summarize_numeric_metrics(fold_df, metric_cols)
    overall_metric_summary.to_csv(save_dir / "overall_metric_summary.csv", index=False)

    parameter_frequency = build_parameter_frequency_table(fold_df)
    parameter_frequency.to_csv(save_dir / "parameter_frequency.csv", index=False)

    fold_df_regime = add_regime_bucket(fold_df)
    by_year = summarize_by_group(fold_df_regime, "test_year")
    by_regime = summarize_by_group(fold_df_regime, "regime_bucket")
    by_year.to_csv(save_dir / "by_year_summary.csv", index=False)
    by_regime.to_csv(save_dir / "by_regime_summary.csv", index=False)

    candidate_summary_df = aggregate_candidate_grid(
        all_candidate_df=all_candidate_df,
        max_val_drawdown=args.max_val_drawdown,
    )
    candidate_summary_df.to_csv(save_dir / "candidate_summary.csv", index=False)
    ...
    deployable_compare_df, recommended_cfg = build_deployable_comparison(
        candidate_summary_df=candidate_summary_df,
        current_fixed_cfg=current_fixed_cfg,
    )
    deployable_compare_df.to_csv(save_dir / "deployable_config_comparison.csv", index=False)

    cost_sensitivity_summary = summarize_cost_sensitivity(all_cost_df)
    cost_sensitivity_summary.to_csv(save_dir / "cost_sensitivity_summary.csv", index=False)

    coverage_summary = {
        "data_start_date": str(raw_df["date"].min().date()),
        "data_end_date": str(raw_df["date"].max().date()),
        "n_folds": int(len(fold_df)),
        "n_partial_tail_folds": int(fold_df["is_partial_tail"].sum()) if "is_partial_tail" in fold_df.columns else 0,
    ...
    walkforward_overall_summary = {
        "n_folds": int(len(fold_df)),
        "selected_constraints_satisfied_rate": float(pd.to_numeric(fold_df["selected_constraints_satisfied"], errors="coerce").mean()),
        "positive_selected_test_cumret_rate": float((pd.to_numeric(fold_df["selected_test_cumulative_return"], errors="coerce") > 0).mean()),
        "positive_current_fixed_test_cumret_rate": float((pd.to_numeric(fold_df["current_fixed_test_cumulative_return"], errors="coerce") > 0).mean()),
        "selected_test_cumret_mean": float(pd.to_numeric(fold_df["selected_test_cumulative_return"], errors="coerce").mean()),
        "selected_test_cumret_std": float(pd.to_numeric(fold_df["selected_test_cumulative_return"], errors="coerce").std(ddof=1)) if len(fold_df) > 1 else 0.0,
        "selected_test_sharpe_mean": float(pd.to_numeric(fold_df["selected_test_sharpe"], errors="coerce").mean()),
        "selected_test_sharpe_std": float(pd.to_numeric(fold_df["selected_test_sharpe"], errors="coerce").std(ddof=1)) if len(fold_df) > 1 else 0.0,
        "selected_test_mdd_mean": float(pd.to_numeric(fold_df["selected_test_max_drawdown"], errors="coerce").mean()),
        "selected_test_mdd_std": float(pd.to_numeric(fold_df["selected_test_max_drawdown"], errors="coerce").std(ddof=1)) if len(fold_df) > 1 else 0.0,
        ...
        "current_fixed_test_cumret_mean": float(pd.to_numeric(fold_df["current_fixed_test_cumulative_return"], errors="coerce").mean()),
        "current_fixed_test_sharpe_mean": float(pd.to_numeric(fold_df["current_fixed_test_sharpe"], errors="coerce").mean()),
        "current_fixed_test_mdd_mean": float(pd.to_numeric(fold_df["current_fixed_test_max_drawdown"], errors="coerce").mean()),
        "selected_matches_current_fixed_rate": float(pd.to_numeric(fold_df["selected_matches_current_fixed"], errors="coerce").mean()),
    }

    final_manifest = {
        "final_system_name": "H5 Transformer v5 final deployable system",
        "final_decision": "freeze_recommended_deployable_config" if recommended_cfg else "no_recommendation_generated",
        "frozen_config": {
            "mode": recommended_cfg.get("mode"),
            "top_k": int(recommended_cfg["top_k"]) if "top_k" in recommended_cfg else None,
            "min_prob": float(recommended_cfg["min_prob"]) if "min_prob" in recommended_cfg else None,
            "threshold": float(recommended_cfg["threshold"]) if "threshold" in recommended_cfg else None,
        } if recommended_cfg else {},
        "replaced_previous_config": current_fixed_cfg,
        "transaction_cost_bps": float(args.transaction_cost_bps),
        "horizon": int(horizon),
        "diagnostics_dir": str(save_dir.resolve()),
        "coverage_summary": coverage_summary,
        "walkforward_overall_summary": walkforward_overall_summary,
        "feature_count": int(len(feats)),
        "data_path": str(Path(args.data_path).resolve()),
        "run_args": vars(args),
    }

    with open(save_dir / "final_system_manifest.json", "w", encoding="utf-8") as f:
        json.dump(final_manifest, f, ensure_ascii=False, indent=2)

    print("\n" + "=" * 70)
    print("Finished deployable diagnostics.")
    print(f"Saved outputs to: {save_dir}")
    if recommended_cfg:
        print("Recommended frozen config:")
        print(json.dumps(final_manifest["frozen_config"], ensure_ascii=False, indent=2))
    else:
        print("No recommended config was generated.")
