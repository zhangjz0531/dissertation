...
def align_price_and_sentiment_smart():
    print("[+] 开始智能对齐 SPY K线数据与 VIX 情绪数据...")

    BASE_DIR = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    dataload_dir = os.path.join(BASE_DIR, "dataload")

    # 🚨 1. 读取最新的 SPY_1d 数据
    spy_path = os.path.join(dataload_dir, 'SPY_1d.csv')
    fng_path = os.path.join(dataload_dir, 'FNG_Sentiment_1D.csv')

    if not os.path.exists(spy_path) or not os.path.exists(fng_path):
    ...
    # 2. 统一时间格式
    if 'start_time' in df_spy.columns:
        df_spy['date'] = pd.to_datetime(df_spy['start_time']).dt.strftime('%Y-%m-%d')
    else:
        df_spy['date'] = pd.to_datetime(df_spy['Date']).dt.strftime('%Y-%m-%d')

    df_fng['date'] = df_fng['date'].astype(str)

    # 3. 🧠 智能侦测
    missing_dates = df_spy[~df_spy['date'].isin(df_fng['date'])]['date'].tolist()
    if missing_dates:
    ...
    # 4. 以资产为基准进行左连接
    df_merged = pd.merge(df_spy, df_fng[['date', 'fng_value', 'sentiment_score']], on='date', how='left')
    ...
    # 🚨 输出正名为 SPY_1d_with_Sentiment.csv
    output_file = os.path.join(dataload_dir, 'SPY_1d_with_Sentiment.csv')
